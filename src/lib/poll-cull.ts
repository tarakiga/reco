...
export function topTierGenres(
  ...
  for (const v of votes) {
    const t = titleMap.get(v.optionKey);
    if (!t) continue;
...
/**
 * Survivors of the round-1 genre cull, ordered by vote count then option.
 * An option survives if any of its genres is in the top tier. When every pick is
