...
import { addFavourite, removeFavourite, listFavourites } from "@/services/user-catalog";
...
export async function GET() {
  ...
  return NextResponse.json({ items: await listFavourites(profile.id) });
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    await addFavourite(profile.id, title.id);
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    const input = titleRef.parse(body);
    const title = await getOrCreateTitle(input.mediaType, input.tmdbId);
    await removeFavourite(profile.id, title.id);
    return NextResponse.json({ ok: true });
  } catch (err) {
    if (err instanceof ZodError) return jsonError(400, "Validation failed", err.issues);
    console.error(err);
