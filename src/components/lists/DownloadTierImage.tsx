...
export function DownloadTierImage({
  ...
  return (
    <button
      ...
      disabled={loading}
      aria-busy={loading}
      className="inline-flex items-center gap-1.5 rounded-md border border-border bg-surface px-3 py-2 text-sm font-medium text-text transition-colors hover:border-accent hover:text-accent-text disabled:opacity-60"
    >
      {loading ? (
